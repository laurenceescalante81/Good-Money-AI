...
export default function PlanningScreen() {
  ...
  const { isDesktop, isTablet, isMobile, chartWidth, contentWidth, sidePadding } = useResponsive();
  ...
  const wealthChartH = isDesktop ? 380 : isTablet ? 300 : 220;
  const incExpChartH = isDesktop ? 320 : isTablet ? 240 : 180;

  return (
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: isMobile ? 100 : 60 }}>
        <CoinHeader title="Planning" />

        <View style={[styles.contentWrap, { maxWidth: contentWidth, paddingHorizontal: sidePadding }]}>
          ...
          <View style={[styles.summaryRow, isDesktop && styles.summaryRowDesktop]}>
            <View style={[styles.summaryCard, { backgroundColor: Colors.light.tint + "12" }, isDesktop && { flex: 1 }]}>
              <Text style={[styles.summaryLabel, { fontSize: fs(11) }]}>Net Wealth</Text>
              <Text style={[styles.summaryVal, { color: Colors.light.tint, fontSize: fs(isDesktop ? 24 : 20) }]}>
                {fmt(wealthProjection[0]?.wealth || 0)}
              </Text>
            </View>
            <View style={[styles.summaryCard, { backgroundColor: Colors.light.income + "12" }, isDesktop && { flex: 1 }]}>
              <Text style={[styles.summaryLabel, { fontSize: fs(11) }]}>Monthly Savings</Text>
              <Text style={[styles.summaryVal, { color: monthlySavings >= 0 ? Colors.light.income : Colors.light.expense, fontSize: fs(isDesktop ? 24 : 20) }]}>
                {monthlySavings >= 0 ? "+" : "-"}{fmt(Math.abs(monthlySavings))}
              </Text>
            </View>
            {isDesktop && (
              <>
                <View style={[styles.summaryCard, { backgroundColor: Colors.light.mortgage + "12", flex: 1 }]}>
                  <Text style={[styles.summaryLabel, { fontSize: fs(11) }]}>Property Equity</Text>
                  <Text style={[styles.summaryVal, { color: Colors.light.mortgage, fontSize: fs(24) }]}>
                    {fmt(wealthProjection[0]?.property || 0)}
                  </Text>
                </View>
                <View style={[styles.summaryCard, { backgroundColor: Colors.light.super + "12", flex: 1 }]}>
                  <Text style={[styles.summaryLabel, { fontSize: fs(11) }]}>Super Balance</Text>
                  <Text style={[styles.summaryVal, { color: Colors.light.super, fontSize: fs(24) }]}>
                    {fmt(wealthProjection[0]?.super_ || 0)}
                  </Text>
                </View>
              </>
            )}
          ...
          <View style={{ position: 'relative' }}>
            <View style={!isFactFindComplete ? { opacity: 0.25 } : undefined} pointerEvents={isFactFindComplete ? 'auto' : 'none'}>
              <View style={styles.section}>
                <Text style={[styles.sectionTitle, { fontSize: fs(isDesktop ? 20 : 16) }]}>Net Wealth Projection</Text>
                ...
                <NetWealthChart data={wealthProjection} width={chartWidth} height={wealthChartH} />
                ...
                <View style={styles.projectedCard}>
                  ...
                  <Text style={[styles.projectedVal, { fontSize: fs(isDesktop ? 24 : 20) }]}>{fmt(wealthProjection[wealthProjection.length - 1]?.wealth || 0)}</Text>
                </View>
              </View>

              <View style={[styles.chartsRow, isDesktop && styles.chartsRowDesktop]}>
                <View style={[styles.section, isDesktop && { flex: 1 }]}>
                  <Text style={[styles.sectionTitle, { fontSize: fs(isDesktop ? 20 : 16) }]}>Income vs Expenses</Text>
                  <Text style={[styles.sectionSubtitle, { fontSize: fs(12) }]}>Last 6 months</Text>
                  <IncomeExpenseChart data={incomeExpenseData} width={isDesktop ? Math.floor(chartWidth * 0.55) : chartWidth} height={incExpChartH} />
                  <View style={styles.legendRow}>
                ...
                <View style={[styles.section, isDesktop && { flex: 1, maxWidth: '40%' as any }]}>
                  <Text style={[styles.sectionTitle, { fontSize: fs(isDesktop ? 20 : 16) }]}>Wealth Breakdown</Text>
            ...
            {!isFactFindComplete && (
              <View style={styles.ctaOverlay}>
                <View style={[styles.ctaCard, isDesktop && { maxWidth: 520 }]}>
...
function NetWealthChart({ data, width, height }: { data: { year: number; wealth: number; property: number; super_: number; savings: number }[]; width: number; height: number }) {
  const { fs } = useAccessibility();
  const chartH = height;
  const padL = 60;
  const padR = 20;
  ...
  const gridLines = chartH > 300 ? 7 : 5;
      ...
      <Svg width={width} height={chartH}>
        ...
        {gridVals.map((v, i) => (
          <React.Fragment key={i}>
            ...
            <SvgText x={padL - 8} y={toY(v) + 4} fontSize={fs(10)} fill={Colors.light.textMuted} textAnchor="end" fontFamily="DMSans_500Medium">{fmtK(v)}</SvgText>
          </React.Fragment>
        ))}
        ...
        {data.filter((_, i) => i === 0 || i === data.length - 1).map((d, idx) => (
          <Circle key={idx} cx={toX(idx === 0 ? 0 : data.length - 1)} cy={toY(d.wealth)} r={5} fill={Colors.light.tint} stroke="#fff" strokeWidth={2} />
        ))}
        {data.filter((_, i) => i % Math.max(1, Math.floor(data.length / 6)) === 0 || i === data.length - 1).map((d, idx, arr) => {
          ...
          return (
            <SvgText key={idx} x={toX(origIdx)} y={chartH - 8} fontSize={fs(10)} fill={Colors.light.textMuted} textAnchor="middle" fontFamily="DMSans_500Medium">
              Yr {d.year}
            </SvgText>
          );
        })}
...
function IncomeExpenseChart({ data, width, height }: { data: { months: string[]; incomes: number[]; expenses: number[] }; width: number; height: number }) {
  const { fs } = useAccessibility();
  const chartH = height;
  const padL = 60;
  const padR = 20;
  ...
  const barW = Math.min(24, chartW / data.months.length / 3);
  const gap = 4;

  const gridLines = chartH > 240 ? 6 : 4;
        ...
        {data.months.map((month, i) => {
          ...
          return (
            <React.Fragment key={i}>
              ...
              <SvgText x={cx} y={chartH - 8} fontSize={fs(10)} fill={Colors.light.textMuted} textAnchor="middle" fontFamily="DMSans_500Medium">{month}</SvgText>
            </React.Fragment>
          );
        })}
      </Svg>
...
const styles = StyleSheet.create({
  ...
  chartContainer: { backgroundColor: Colors.light.card, borderRadius: 16, padding: 10, marginBottom: 12 },
  ...
  chartsRow: {},
  chartsRowDesktop: { flexDirection: 'row', gap: 24 },
  breakdownList: { gap: 8 },
  ...
  ctaOverlay: { position: "absolute" as const, top: 40, left: 0, right: 0, zIndex: 10, alignItems: "center" as const },
