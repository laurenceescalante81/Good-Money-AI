  ...
  return (
    <View style={[styles.container, { paddingTop: topInset }]}>
      <View style={styles.topBar}>
        ...
        <Pressable onPress={handleSave} disabled={!canSave} hitSlop={12}><Ionicons name="checkmark" size={26} color={canSave ? Colors.light.budget : Colors.light.gray300} /></Pressable>
      </View>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: bottomInset + 20 }}>
        <View style={styles.section}>
          <Text style={styles.label}>Goal Name</Text>
          <TextInput style={styles.nameInput} placeholder="e.g. Holiday to Bali" placeholderTextColor={Colors.light.textMuted} value={name} onChangeText={setName} autoFocus />
        </View>
        <View style={styles.section}>
          <Text style={styles.label}>Target Amount (AUD)</Text>
          <View style={styles.amountRow}>
            <Text style={styles.dollar}>$</Text>
            <TextInput style={styles.amountInput} placeholder="5000" placeholderTextColor={Colors.light.gray300} keyboardType="decimal-pad" value={targetAmount} onChangeText={setTargetAmount} />
          </View>
        </View>
        <View style={styles.presetRow}>
          {[1000, 5000, 10000, 25000].map(v => (
            <Pressable key={v} onPress={() => setTargetAmount(v.toString())} style={[styles.presetBtn, targetAmount === v.toString() && styles.presetActive]}>
              <Text style={[styles.presetText, targetAmount === v.toString() && styles.presetTextActive]}>${v >= 1000 ? `${v / 1000}k` : v}</Text>
            </Pressable>
          ))}
        </View>
          ...
          <View style={styles.iconGrid}>
            {GOAL_ICONS.map(g => (
              <Pressable key={g.icon} onPress={() => setSelectedIcon(g.icon)} style={[styles.iconBtn, selectedIcon === g.icon && styles.iconBtnActive]}>
                <Ionicons name={g.icon as any} size={24} color={selectedIcon === g.icon ? Colors.light.white : Colors.light.budget} />
              </Pressable>
            ))}
          </View>
        </View>
        {canSave && (
          <View style={styles.previewCard}>
            <Ionicons name={selectedIcon as any} size={24} color={Colors.light.budget} />
            <View style={{ flex: 1 }}>
              <Text style={styles.previewName}>{name}</Text>
              <Text style={styles.previewSub}>Save ${(parseFloat(targetAmount) / parseInt(months || "6")).toFixed(0)}/month</Text>
            </View>
          </View>
        )}
      </ScrollView>
